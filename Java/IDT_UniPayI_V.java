...
public class IDT_UniPayI_V extends IDT_Device implements Configurable, MSRInterface, ICCInterface, EMVFunctional {
	 ...
	 * 
	 */
	public IDT_UniPayI_V(OnReceiverListener callback) {
		IDTechSDKBridge.m_recListener = callback;
		//IDTechSDKBridge.jdevicesetCurrentDevice(DEVICE_TYPE.IDT_DEVICE_UNIPAYI_V.getNumVal());
	}
	
	/**
	 * Defines connection USB or Audio Jack
	 * 
	 * @param deviceType  DEVICE_TYPE.IDT_DEVICE_UNIPAYI_V
	 * 
	 */
	public boolean device_setDeviceType(ReaderInfo.DEVICE_TYPE deviceType) {
		if (deviceType == DEVICE_TYPE.IDT_DEVICE_UNIPAY_I_V) {
			int rc = IDTechSDKBridge.jdeviceinit();
			if (rc != 0) return false;
			return IDTechSDKBridge.jdevicesetCurrentDevice(DEVICE_TYPE.IDT_DEVICE_UNIPAY_I_V.getNumVal());
		}
		return false;
	}	
