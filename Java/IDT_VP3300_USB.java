...
public class IDT_VP3300_USB extends IDT_Device implements Configurable, MSRInterface, ICCInterface, EMVFunctional, CTLSFunctional, ThreeInterfaceDevice {
	 ...
	 * 
	 */
	public IDT_VP3300_USB(OnReceiverListener callback) {
		IDTechSDKBridge.m_recListener = callback;
		//IDTechSDKBridge.jdevicesetCurrentDevice(DEVICE_TYPE.IDT_DEVICE_VP3300_USB.getNumVal());
	}
	
	/**
	 * Defines connection USB or Audio Jack
	 * 
	 * @param deviceType  DEVICE_TYPE.IDT_DEVICE_VP3300_USB
	 * 
	 */
	public boolean device_setDeviceType(ReaderInfo.DEVICE_TYPE deviceType) {
		if (deviceType == DEVICE_TYPE.IDT_DEVICE_VP3300_USB) {
			int rc = IDTechSDKBridge.jdeviceinit();
			if (rc != 0) return false;
			return IDTechSDKBridge.jdevicesetCurrentDevice(DEVICE_TYPE.IDT_DEVICE_VP3300_USB.getNumVal());
		}
		return false;
	}	
