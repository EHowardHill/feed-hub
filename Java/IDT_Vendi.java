...
public class IDT_Vendi extends IDT_Device implements Configurable, MSRInterface, CTLSFunctional {
	
	/**
	 * It is the constructor of the main class IDT_Vendi. When it is
	 * called, the SDK will create the Instance for IDT_Vendi device. The
	 ...
	 * 
	 */
	public IDT_Vendi(OnReceiverListener callback) {
	 ...
	 * 
	 * @param deviceType  DEVICE_TYPE.IDT_DEVICE_VENDI
	 * 
	 */
	public boolean device_setDeviceType(ReaderInfo.DEVICE_TYPE deviceType) {
		if (deviceType == DEVICE_TYPE.IDT_DEVICE_VENDI) {
			int rc = IDTechSDKBridge.jdeviceinit();
			if (rc != 0) return false;
			return IDTechSDKBridge.jdevicesetCurrentDevice(DEVICE_TYPE.IDT_DEVICE_VENDI.getNumVal());
		}
		return false;
	}	
